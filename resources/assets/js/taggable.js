...
window.Taggable = (contextUsers, maxLength = null) => {
    const tribute = new Tribute({
        ...
        containerClass: 'UserTagger bg-white rounded-md shadow-lg py-6 z-50 left-0 sm:left-auto w-full sm:w-auto',
        ...
        menuItemTemplate (item) {
            ...
            return `<div class="flex p-3 space-x-3 items-center cursor-pointer">
                <div class="w-6">
                    <div class="relative w-full overflow-hidden rounded border border-theme-secondary-300 ${hasAvatar ? 'pb-full h-0' : ''}">
                        ${avatar}
                    </div>
                </div>
                <span class="UserTagger--username font-semibold text-theme-secondary-800">${item.original.username}</span>
                <span class="text-sm text-theme-secondary-500">${item.original.name}</span>
            </div>`;
        },
