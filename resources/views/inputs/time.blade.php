<div x-data="{ actionOpen: false, value: '{{ $time ?? '00:00' }}' }" class="flex-1">
    <x-ark-dropdown
        ...
        button-class="form-select shadow-sm rounded w-full"
    ...
    >
        ...
        <div class="items-center justify-center block text-theme-secondary-900 h-40 overflow-y-scroll dropdown-scrolling">
            @for ($hour = 0; $hour < 24; $hour++)
                @for ($minute = 0; $minute < 60; $minute += 30)
                    @php($value = str_pad($hour, 2, '0', STR_PAD_LEFT).':'.str_pad($minute, 2, '0', STR_PAD_LEFT))

                    <div
                        class="cursor-pointer justify-center dropdown-entry py-2"
                        @click="value='{{ $value }}'"
                    >
                        {{ $value }}
                    </div>
